...
class DiskUtil
{
    ...
    {
        [$disk, $ruta] = self::obtenerDiscoRuta($rutaOrig);
    ...
    public static function ensureDirExists($dir)
    {
        ...
        [$disk, $ruta] = self::obtenerDiscoRuta($dir);
    ...
    public static function normalizePath($ruta)
    {
        ...
        if (preg_match("/^\/storage\//", $ruta)) {
            return preg_replace("#^\/storage\/?#", '/almacen/', $ruta);
        }
        if (preg_match("/^\/almacen\//", $ruta)) {
            return $ruta;
        }
