...
@php
  ...
  $className = class_basename($entry);
  $coleccion = strtolower($className);
  $coleccion .= substr($coleccion, -1) == 'n' ? 'es' : 's';
  $contenidoUrl = '/' . $coleccion . '/' . $entry->id;
@endphp
...
@section('header')
  <section class="header-operation container-fluid animated fadeIn d-flex mb-2 align-items-end" bp-section="page-header">
    ...
    @endif
    <p class="ms-auto ml-auto mb-2" bp-section="page-subheading-back-button">
      <small><a href="{{ $contenidoUrl }}" target="_blank" class="hidden-print font-sm"><i class="la la-eye"></i> Ver contenido <i class="la la-external-link"></i></a></small>
    </p>
  </section>
@endsection
