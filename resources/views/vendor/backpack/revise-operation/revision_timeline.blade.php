    ...
    <div class="card timeline-item-wrap mb-3">
      ...
      @else
        ...
        <div class="card-body">
          @php
            $showDiff = false;
            $rawOld = $history->old_value ?? '';
            $rawNew = $history->new_value ?? '';
            if (
                $rawOld && $rawNew &&
                !str_ends_with($history->key, '_id') &&
                (strlen($rawOld) > 120 || strlen($rawNew) > 120 || str_contains($rawOld, "\n") || str_contains($rawNew, "\n"))
            ) {
                $showDiff = true;
            }
          @endphp
          ...
          @if($showDiff)
            <div style="font-weight:600;margin-bottom:4px;color:#666">Cambios:</div>
            {!! \App\Helpers\DiffRenderer::render($rawOld, $rawNew) !!}
          @else
            <div class="row">
