...
<template>
    <div class="max-w-screen-xl px-4 py-12 mx-auto sm:px-6 lg:px-8 bg-white">
        <h1>Centros</h1>
        <p>Centros que recogen toda la información de las conversaciones interdimensionales mantenidas con nuestros Guías Estelares.</p>
        <div class="w-full flex gap-5 flex-wrap md:flex-nowrap">
            ...
            <div>
                <div class="card shadow rounded p-10 space-y-7">
                    <ul>
                        ...
                        <li v-for="pais of paises" :key="pais.nombre" class="flex gap-2"
                        >
            ...
            <div class="w-full flex-grow">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8">
                    <div v-if="listado.data.length > 0" v-for="centro in listado.data" :key="centro.id"
                        ...
                        <img :src="centro.imagen_url" :alt="centro.nombre" class="h-48 object-cover w-full" />
                            ...
                            <p class="inline-block text-xs bg-gray-500 text-gray-50 rounded-full py-1 px-3">{{ centro.pais}}</p>
                            <p class="inline-block text-xs bg-gray-500 text-gray-50 rounded-full py-1 px-3">{{ centro.poblacion}}</p>
