<template>
      ...
      <div class="flex flex-col md:flex-row mb-4">
        ...
        <div class="w-full md:w-1/2">
          ...
          <template v-if="showGoogleMapsLink">
            <p class="text-lg">
              <a :href="getGoogleMapsLink(contacto)" target="_blank" rel="noopener noreferrer">Ver en Google Maps</a>
            </p>
          </template>
          ...
          <p v-if="contacto.redes" class="text-lg"><strong>Redes Sociales:</strong> <a :href="contacto.redes">{{ contacto.redes }}</a></p>
        </div>
      </div>
    </div>
  </template>
  ...
  <style scoped>
  /* Agrega estilos personalizados según tus preferencias */
  </style>
