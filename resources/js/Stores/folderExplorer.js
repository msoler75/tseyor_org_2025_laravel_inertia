...
// Creamos un proxy para combinar state y metodos
const storeProxy = new Proxy({...metodos, ...state}, {
    get(target, property) {
      // Si la propiedad está en metodos, devuelve esa
      if (property in metodos) {
        return metodos[property];
      }
      // Si no, devuelve la propiedad del state
      return state[property];
    },
    set(target, property, value) {
      // Siempre actualiza el state
