  ...
  actions: {
    ...
    async play(src, title, options = {}) {
      if (src.match(/\.(mp4|webm|ogv|mkv|mov|avi|wmv|flv|m4v)$/i)) {
        this.audio.pause();
        const hasVideo = await this.checkVideo(src);
        if (!hasVideo) {
          this.playAudio(src, title, options);
        } else {
          // lo abre en una nueva pestaña
          this.playVideo(src, title, options);
        }
        return;
      }
      // si es un audio:
      else if (this.isPlayable(src)) {
        this.playAudio(src, title, options);
      }
    },
...
// Instancia global del store para uso fuera de componentes
// Variables para gestión de eventos globales (solo en cliente)
// Estas NO causan contaminación en SSR porque solo se usan cuando typeof window !== 'undefined'
...
export default function usePlayer() {
  ...
  // Inicializar el store la primera vez que se usa (SOLO en cliente, no en SSR)
  if (typeof window !== 'undefined' && window.Audio) {
    store.init();

    // Instalar eventos globales solo una vez por sesión del navegador
    if (!eventsInstalled) {
      installEvents(store);
      document.addEventListener("keydown", (event) => _handleKeyPress(event, store));
