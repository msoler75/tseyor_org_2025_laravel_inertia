<template>
    <Icon
        ...
        class="cursor-pointer transition duration-100 hover:text-orange-500"
...
watch(
    ...
    (y) => {
        if (prevY != 10000) {
            var dy = y - prevY;
            if (y < heightToShow) show.value = false;
            else if (dy > 0) {
                if (subiendo) {
                    recorridoDown = dy;
                } else {
                    recorridoDown += dy;
                    if (recorridoDown > wrapToHide) show.value = false;
                }
                subiendo = false;
                // bajando
            } else {
                ...
                if (!subiendo) {
                    ...
                    recorridoUp += dy;
                    if (recorridoUp < -1 * wrapToShow) show.value = true;
                }
                subiendo = true;
            }
        }
