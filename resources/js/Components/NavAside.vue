        ...
        <div class="fixed top-0 left-0 h-full bg-gray-100 shadow-lg  transition duration-200 z-50 overflow-y-auto"
            ...
            <div
                ...
                <div
                    ...
                    <button class="flex jusitfy-start items-center space-x-6 w-full  focus:outline-none     rounded ">
                        <Icon icon="ion:chevron-up-outline" />
                        <p class="text-base leading-4 ">Dashboard</p>
                    </button>
                    <button class="flex jusitfy-start items-center w-full  space-x-6    rounded ">
                        <Icon icon="ion:chevron-up-outline" />
                ...
                <div class="flex flex-col justify-start items-center   px-6 border-b border-gray-600 w-full">
                    <template v-for="tab, index in nav.items" :key="index">
                        <button v-if="tab.submenu" @click="nav.toggleTab(tab)"
                            class="focus:outline-none  text-left   flex justify-between items-center w-full py-5 space-x-14  ">
                        ...
                        <Collapse as="section" v-if="tab.submenu" :when="!!tab.open"
                            ...
                            <template v-for="section of tab.submenu.sections" :key="section.title">
                                <button v-for="item of section.items" :key="item.url"
                                    class="flex justify-start items-center space-x-6   rounded px-3 py-5  w-full md:w-52">
                                    <Icon :icon="item.icon" />
